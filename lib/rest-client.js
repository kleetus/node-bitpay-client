...
var RESTClient = function(privateKey, options) {
  ...
  var defaultConf = this.setConfig();
...
/* If not passing in an object specifying operating parameters, then:
** a set of default parameters be will used, either from a set of environment variables,
** a file on the file system or a default set that uses the BitPay test environment.
** 1. If the env varible 'USE_BITPAY_TESTNET' is set and truthy, then the hard-coded default config
** is used for the BitPay test env. No further config options are checked.
** 2. If the 'USE_BITPAY_TESTNET' is not set or falsy, then the env varables, 'BITPAY_API_HOST',
** 'BITPAY_API_PORT', 'BITPAY_FORCE_SSL' are considered. If any of those are set, those values are used.
** 3. If any of those env variables are not set, then a config file from the filesystem is consulted for values.
*/
RESTClient.prototype.setConfig = function() {
  var isWin = (process.platform === 'win32' || process.platform === 'win64');
  var HOME = process.env[isWin ? 'USERPROFILE' : 'HOME'];

  var apiHost = process.env.BITPAY_API_HOST;
  var apiPort = process.env.BITPAY_API_PORT;
  var forceSSL = process.env.BITPAY_FORCE_SSL;
  var useBitpayTestnet = process.env.USE_BITPAY_TESTNET;
  var configFile = HOME + '/.bitpay/config.json';

  defaultConf = sanitizeEnvVariables();
  if (useBitpayTestnet &&
    useBitpayTestnet !== 'false' &&
    useBitpayTestnet !== '0') {
    defaultConf = {
    ...
    var options = sanitizeVariables(apiPort, forceSSL);
    defaultConf  = {
      apiHost: apiHost,
      apiPort: options.port,
      forceSSL: options.ssl
    };
    defaultConf = readConfig(configFile, defaultConf);
  }
  ...
  function sanitizeVariables() {
    ...
    if (typeof port === 'string') {
      ...
      if (/[^\d+]/.test(port)) {
        throw new Error('BITPAY_API_PORT provided is not resemble an integer');
      }
    ...
    if (typeof ssl === 'string') {
      if (ssl === 'true' || ssl === '1') {
        opts.ssl = true;
      } else if (ssl === 'false' || ssl === '0') {
        opts.ssl = false;
      } else {
        throw new Error('BITPAY_FORCE_SSL provided does not resemble a boolean (e.g. "0", "1", "false", "true"');
      }
    }
    return opts;
  }

  function readConfig(file, defaultConf) {
    var conf = require(configFile);
    var keys = Object.keys(conf);
    for (var i = 0; i < keys.length; ++i) {
      if (typeof defaultConf[keys[i]] === 'undefined') {
        defaultConf[keys[i]] = conf[keys[i]];
      }
    }
    return defaultConf;
  }
...
RESTClient.prototype._processResponse = function(entity, path) {
  ...
  path        = hasIdInPath ? (path + '/') : (path + '/' + identifier);
